...
export const Navbar = ({ children, className, visible, setVisible }: NavbarProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(" sticky inset-x-0 lg:top-4 z-40 w-full h-0", className)}
    >
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
   return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-background px-4 py-2 lg:flex dark:bg-transparent",
    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className={cn("relative font-bold px-4 py-2 transition text-secondary", hovered === idx && "text-primary-foreground")}
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
